...
function updateCart() {
  ...
  cart.position.x = camPos.x + fwd.x * 0.2;
  cart.position.z = camPos.z + fwd.z * 0.2;
  cart.position.y = 0.2;
...
function initFrozenQueue() {
  ...
  if (!sw || sw.length < 3) {
    console.warn('⚠️ Frozen Queue: لم يتم العثور على 3 شخصيات متجمدة');
    return;
  }
...
function shiftQueueIfNeeded() {
  ...
  if (remaining.length === 0) {
    _fqPeople = [];
    _fqActive = false;
    setTimeout(() => spawnEntireQueue(), 2000);
    return;
  }
  ...
  for (let i = 0; i < remaining.length; i++) {
    ...
    if (dist > 0.05) {
      mesh.position.x += dx * 0.15;
      mesh.position.z += dz * 0.15;
    } else {
