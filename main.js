...
function fillShelfThin(productConfig) {
  ...
  loadModel(model, [0,-9999,0], [0,0,0], [1,1,1]).then((src) => {
    ...
    // نجمع كل الـ meshes في mesh واحد عشان نضمن إن الموديل بيظهر كامل
    const childMeshes = src.getChildMeshes().filter(m => m.getTotalVertices() > 0);
    if (!childMeshes.length) { src.dispose(); return; }

    let baseMesh;
    if (childMeshes.length === 1) {
      baseMesh = childMeshes[0];
    } else {
      // دمج كل الـ meshes في mesh واحد
      baseMesh = BABYLON.Mesh.MergeMeshes(childMeshes, true, true, undefined, false, true);
...
function fillFridgeThin(fridgePos, fridgeRot, fridgeScale, drinkConfig) {
  ...
  loadModel(drinkConfig.model, [0,-9999,0], [0,0,0], [1,1,1]).then((src) => {
    ...
    const childMeshes2 = src.getChildMeshes().filter(m => m.getTotalVertices() > 0);
    if (!childMeshes2.length) { src.dispose(); return; }

    let baseMesh;
    if (childMeshes2.length === 1) {
      baseMesh = childMeshes2[0];
    } else {
      baseMesh = BABYLON.Mesh.MergeMeshes(childMeshes2, true, true, undefined, false, true);
      if (!baseMesh) { src.dispose(); return; }
    }
...
async function loadVegetablesThin(vegConfig) {
  for (const item of vegConfig.items) {
    ...
    const childMeshes3 = src.getChildMeshes().filter(m => m.getTotalVertices() > 0);
    if (!childMeshes3.length) { src.dispose(); continue; }

    let baseMesh;
    if (childMeshes3.length === 1) {
      baseMesh = childMeshes3[0];
    } else {
      baseMesh = BABYLON.Mesh.MergeMeshes(childMeshes3, true, true, undefined, false, true);
