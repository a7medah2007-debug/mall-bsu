...
// ✅ المنتج ينتقل مباشرة للسلة (بدون إمساك باليد)
...
function setupRightMC(mc, controller) {
  ...
  if (trigger) {
    trigger.onButtonStateChangedObservable.add((comp) => {
      ...
      if (comp.pressed) window.grabAndAddToCart?.();
    });
  }
...
function pollXRControllersUnified(xr) {
  ...
  for (const ctrl of controllers) {
    ...
    if (buttons?.length >= 2) {
      ...
      if (hand === 'right') {
        if (t && !prev.t) window.grabAndAddToCart?.();
        if (s && !prev.s) window.grabAndAddToCart?.();
      }
      if (hand === 'left') {
        if (t && !prev.t) {
          if (!window.attachedCart) window.grabCart?.();
