...
async function loadAssets() {
  ...
  // تشغيل طابور الكاشير بعد تحميل الشخصيات
...
function pollXRControllersUnified(xr) {
  ...
  for (const ctrl of controllers) {
    ...
    if (buttons?.length >= 2) {
      const t = buttons[0]?.pressed ?? false;
      const s = buttons[1]?.pressed ?? false;

      if (hand === 'right') {
        if (t && !prev.t) window.grabNearestProduct?.();
        if (!t && prev.t && window.heldItem) window.addToCart?.();
        if (s && !prev.s) {
          if (window.heldItem) window.throwItem?.();
          else window.grabNearestProduct?.();
        }
      }
      if (hand === 'left') {
        if (t && !prev.t) {
        ...
        if (s && !prev.s) window.payAtCashier?.();
      }
