...
function setupLeftMC(mc, controller) {
  ...
  // ✅ Squeeze يسار: إنهاء الدفع وحساب الباقي
  const squeeze = mc.getComponent('xr-standard-squeeze');
  if (squeeze) {
    squeeze.onButtonStateChangedObservable.add((comp) => {
      if (!comp.changes?.pressed || !comp.pressed) return;
...
// ── loop مسك الفكة — 0.5 متر من أي حاجة (كاميرا أو يد يمين أو يد يسار) ──
scene.registerBeforeRender(() => {
  ...
  const billPos = firstBill.position;

  // جمع كل النقاط الممكنة (كاميرا + إيد يمين + إيد يسار)
  const points = [];

  const cam = window._xrCamera ?? window.camera;
  if (cam) points.push(cam.globalPosition ?? cam.position);

  if (window._rightGrip) points.push(window._rightGrip.absolutePosition ?? window._rightGrip.position);
  if (window._leftGrip)  points.push(window._leftGrip.absolutePosition  ?? window._leftGrip.position);

  // نشوف أقرب نقطة للفلوس
  for (const pt of points) {
    if (!pt) continue;
    if (BABYLON.Vector3.DistanceSquared(pt, billPos) < 0.25) {  // 0.5 متر
